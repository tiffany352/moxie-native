...
#[derive(Clone)]
struct Notifier {
    events_proxy: EventLoopProxy<Message>,
    tx: mpsc::Sender<()>,
}
...
impl RenderNotifier for Notifier {
    ...
    fn wake_up(&self) {
        self.tx.send(()).unwrap();
        let _ = self.events_proxy.send_event(Message::WebrenderReady);
    }
...
impl Context {
    ...
    pub fn set_dom_window(&mut self, new_node: Node<Window>) {
        self.document.set_window(new_node);
    }
    ...
    fn render_child(
        ...
        match layout.render {
            RenderData::Node(ref node) => {
                let values = self.document.get_values(node.into());
            ...
            RenderData::Text {
                ...
                let color = {
                    let values = self.document.get_values(parent.into());
                    values.text_color
                };
